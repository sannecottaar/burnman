...
# I change this to 1 to make high quality plots, but computation takes a while...
resample =10
...
for i in range(0,len(pressures),resample):
    ...
    molar_fractions_in_composite, molar_volumes, molar_masses = ulvz_melt_model.calculate_endmember_proportions_volumes_masses(pressures[i], temperatures[i], solid_fe[i], melt_fe[i], melt_frac[i], c_mantle )
    
    
    if melt_frac[i]<1.:
        # construct solid composition
        # fractions of bridgmanite vs periclase
        tot_pv = molar_fractions_in_composite['mpv'] + molar_fractions_in_composite['fpv']
        tot_per =molar_fractions_in_composite['per'] + molar_fractions_in_composite['wus']
        # Set solid solutions
        mg_fe_perovskite.set_composition([molar_fractions_in_composite['mpv']/tot_pv, molar_fractions_in_composite['fpv']/tot_pv,0.])
        mg_fe_periclase.set_composition([molar_fractions_in_composite['per']/tot_per, molar_fractions_in_composite['wus']/tot_per])

        # Set solid composite
        molar_frac_solid = tot_pv+tot_per
        solid = burnman.Composite([mg_fe_perovskite, mg_fe_periclase], [tot_pv/molar_frac_solid, tot_per/molar_frac_solid])
    
    if melt_frac[i]>0.:
        #construct liquid solid solution
        molar_frac_melt = molar_fractions_in_composite['mg_melt']+molar_fractions_in_composite['fe_melt']
        frac_mg_melt =molar_fractions_in_composite['mg_melt']/ molar_frac_melt
        frac_fe_melt =molar_fractions_in_composite['fe_melt']/ molar_frac_melt
    ...
    if melt_frac[i]==1.:
        ...
        mix = burnman.Composite([solid, melt],[molar_frac_solid, molar_frac_melt])
        # choose averaging scheme between melt and solid
        # currently set to a linear approximation for a dihedral angle of 20 and a poisson ratio of 0.25 (roughly fitted to Figure 2 in Takei 2002).
        mix.set_averaging_scheme(average_solid_melt.contiguity_model_linear_approximation())

    # evaluate velocities and density
    [vs_i, vp_i,rho_i] = mix.evaluate(['v_s','v_p','density'], pressures[i], temperatures[i])
...
plt.subplot(2,2,3)
pl= plt.tricontourf(x[::resample]/1.e3,y[::resample]/1.e3,dvs, plot_val, cmap='OrRd_r', vmin = -20., vmax = 0, extend = 'both')
bar= plt.colorbar()
pl.set_clim(-20., 0)
...
plt.subplot(2,2,4)
pl=plt.tricontourf(x[::resample]/1.e3,y[::resample]/1.e3,dvp, plot_val, cmap='OrRd_r', vmin =-20., vmax = 0., extend = 'both' )
bar= plt.colorbar()
...
outfile = 'output/'+model_name+'/solution/seismic_velocities_res'+str(resample)+'.h5'
