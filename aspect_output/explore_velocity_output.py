...
for m,model_name in enumerate(model_names):
        ...
        vel_file = glob.glob('output/'+model_name+'/solution/seismic_velocities_solidonly_res10.h5')[0]
        ...
        plt.subplot(2,2,m+1)
        plt.plot(temperatures[::resample],dvs/dvp,'.b',markersize=0.2, label='including_melt')




        vel_file = glob.glob('output/'+model_name+'/solution/seismic_velocities_res10.h5')[0]
        vels = h5.File(vel_file,'r')
        vs = np.array(vels['vs'])
        vp = np.array(vels['vp'])
        x= np.array(vels['x'])
        ...
        plt.subplot(2,2,m+1)
        plt.plot(temperatures[::resample],dvs/dvp,'.r',markersize=0.2, label='including_melt')
        plt.ylabel('Vs')
