...
class MOD_HP_TMT(eos.EquationOfState):

    """
    Base class for the Holland and Powell (2011) 
    equation of state, itself modified to better 
    deal with minerals at high pressures and temperatures.

    The Holland and Powell (2011) equation of state is itself a correction to
    the generic modified Tait equation of state (class MT).

    An instance "m" of a Mineral can be assigned this
    equation of state with the command m.set_method('mod_hp_tmt')
    ...
    def molar_heat_capacity_pref(self, temperature, params):
        """
        ...
        """
        Cp = params['Cp_Pref'][0] + params['Cp_Pref'][1] * temperature + params['Cp_Pref'][2] * \
            np.power(temperature, -2.) + params['Cp_Pref'][3] * np.power(temperature, -0.5)
        return Cp
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        if pressure != params['Pref']:
            ...
            intVdP = 0.
        return params['H_Pref'] + self.__intCpdT(temperature, params) - temperature * (params['S_Pref'] + self.__intCpoverTdT(temperature, params)) + intVdP
    ...
    def entropy(self, pressure, temperature, volume, params):
        ...
        return params['S_Pref'] + self.__intCpoverTdT(temperature, params) + dintVdpdT
    ...
    def molar_heat_capacity_p(self, pressure, temperature, volume, params):
        ...
        dSdT = dSdT0 + dSdT1
        return self.molar_heat_capacity_pref(temperature, params) + temperature * dSdT
    ...
    def __intCpdT(self, temperature, params):
        ...
        """
        return (params['Cp_Pref'][0] * temperature + 0.5 * params['Cp_Pref'][1] * np.power(temperature, 2.) - params['Cp_Pref'][2] / temperature + 2. * params['Cp_Pref'][3] * np.sqrt(temperature)) - (params['Cp_Pref'][0] * params['T_0'] + 0.5 * params['Cp_Pref'][1] * params['T_0'] * params['T_0'] - params['Cp_Pref'][2] / params['T_0'] + 2.0 * params['Cp_Pref'][3] * np.sqrt(params['T_0']))
    ...
    def __intCpoverTdT(self, temperature, params):
        ...
        at ambient pressure [Pa]
        """
        return (params['Cp_Pref'][0] * np.log(temperature) + params['Cp_Pref'][1] * temperature - 0.5 * params['Cp_Pref'][2] / np.power(temperature, 2.) - 2.0 * params['Cp_Pref'][3] / np.sqrt(temperature)) - (params['Cp_Pref'][0] * np.log(params['T_0']) + params['Cp_Pref'][1] * params['T_0'] - 0.5 * params['Cp_Pref'][2] / (params['T_0'] * params['T_0']) - 2.0 * params['Cp_Pref'][3] / np.sqrt(params['T_0']))
    ...
    def validate_parameters(self, params):
        ...
        if 'H_Pref' not in params:
            params['H_Pref'] = float('nan')
        if 'S_Pref' not in params:
            params['S_Pref'] = float('nan')
        ...
        expected_keys = ['Pref', 'H_Pref', 'S_Pref', 'V_0', 'Cp_Pref', 'a_0', 'n', 'molar_mass']
        ...
        # no test for H_0
        if params['S_Pref'] is not float('nan') and params['S_Pref'] < 0.:
            warnings.warn('Unusual value for S_Pref', stacklevel=2)
        if params['V_0'] < 1.e-7 or params['V_0'] > 1.e-2:
            warnings.warn('Unusual value for V_0', stacklevel=2)

        if self.molar_heat_capacity_pref(params['T_0'], params) < 0.:
            warnings.warn('Negative heat capacity at T_0', stacklevel=2)
        if self.molar_heat_capacity_pref(2000., params) < 0.:
